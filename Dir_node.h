...
public:
   ...
   void find_best_child_sprt(const Leaf_entry &new_data, int* alphabet_sizes, unsigned int& child_block_no, int& child_index, int index, int cur_height);
   ...
   Error_code insert_new_entry_sprt(unsigned int new_entry_block_no, unsigned char* new_entry_DMBR, SplitDimBitBool newWhichDimsSplit, double dir_min_util, int *alphabet_sizes, unsigned char* cur_DMBR,
      Dir_node*& new_dir_node, unsigned char* new_DMBR, int& best_dir_dim);
   ...
   void find_entries_by_box_query(	const Dir_entry box_query_data, vector<int>& block_numbers_in_box, int tmp_height);
   ...
   void set_node_entry_whichDimSplit(int index, SplitDimBitBool newWhichDimsSplit);
   SplitDimBitBool get_node_entry_whichDimSplit(int index);
   ...
   // public data members
   static const int DIR_NODE_SIZE =  (DISK_BLOCK_SIZE - DIR_NODE_OVERHEAD) / sizeof(Dir_entry);
...
private:
   ...
   int greedyMinBalanceSplit(int& alphabetSize, vector<unsigned char>& varAlphabetFreq, bool* minSplitChars, double leaf_min_util, int curMaxAlphabetSize);
   void list_common_split_dims(SplitDimBitBool tempCommonDims, int* commonDims, int& splitDimSize);
   bool is_covered(const Leaf_entry &query_data, unsigned char* DMBR); // check whether the DMBR covers the query data
   bool is_covered_secleced_dim(const Leaf_entry &query_data, unsigned char* DMBR, int* commonDims, int splitDimSize);
   ...
   int howManySplitDimsCovered(const Leaf_entry &query_data, unsigned char* DMBR, SplitDimBitBool splitDimsBool, bool& allDim);
   ...
   int split_algorithm_sprt(Dir_entry* split_entries, double dir_min_util, int* alphabet_sizes, int* group1, int &count1, int* group2, int &count2, unsigned char* DMBR1,
        unsigned char* DMBR2);
   ...
   int split_TO_DEATH_RANDOM_UTIL_v4(Dir_entry* split_entries, double dir_min_util, int* alphabet_sizes, int* group1, int &count1, int* group2, int &count2, unsigned char* DMBR1, unsigned char* DMBR2);
...
void Dir_node::find_entries_by_box_query(
	...
	vector<int>& block_numbers_in_box, int tmp_height)
{
	int i;
	...
	for(i = 0; i < count; i++)
	{
      ...
      if(tmp_height>2){
        //for(int k = 0; k < DMBR_SIZE; k++)   cout<<setw(3)<<static_cast<unsigned int>(entriesVec[i].DMBR[k]);
        //cout<<endl;
...
int Dir_node::howManySplitDimsCovered(const Leaf_entry &query_data, unsigned char* DMBR, SplitDimBitBool splitDimsBool, bool& allDim){
  int i,j,dimensionsMatched=0, byte_no, bit_no;
  SplitDimBitBool temp = 1;
  
  //cout<<"splitDimsBool:"<<splitDimsBool<<endl;
  //cout<<"dimensionsMatched in function:"<<dimensionsMatched<<endl;
  
  allDim = true;
  
  for(i = 0; i < DIM; i++){
    if(splitDimsBool & (temp << i)){
      byte_no = this->DMBR_byte_lut[i][query_data.key[i]];
      bit_no = this->DMBR_bit_lut[i][query_data.key[i]];
      if((DMBR[byte_no] & MASKS[bit_no])){ dimensionsMatched++; }
      else{
          allDim = false;      
      }
      ...
      //byte_no = this->DMBR_byte_lut[i][query_data.key[i]];
      //bit_no = this->DMBR_bit_lut[i][query_data.key[i]];
      //if(DMBR[i] & (1<<query_data.key[i])){ dimensionsMatched++;}
    }
  }
  //cout<<"dimensionsMatched in function end:"<<dimensionsMatched<<endl;
  return dimensionsMatched;
  
}    

void Dir_node::list_common_split_dims(SplitDimBitBool tempCommonDims, int* commonDims, int& splitDimSize){
  
  int i=0;
  
  while(tempCommonDims>0){
    if(tempCommonDims%2!=0){
      commonDims[splitDimSize] = i;
      splitDimSize++;
    }
    tempCommonDims = tempCommonDims >> 1;
    i++;
  }
  
  
}
void Dir_node::find_best_child_sprt(const Leaf_entry &new_data, int* alphabet_sizes, unsigned int& child_block_no, int& child_index, int index, int cur_height){
  
  int i,splitDimsMatched;
  bool logOutput = false, allDim;
  
  int bestAllDimChild = -1, bestAllDimCount = 0, notAllDimMaxChild, bestNotAllDimCount=0;
  
  //if(logOutput){ for(i = 0; i < DIM; i++){ query_result_file<<setw(4)<<static_cast<unsigned int>(new_data.key[i]); } query_result_file<<endl; }
  if(logOutput){ for(i = 0; i < DIM; i++){ cout<<setw(4)<<static_cast<unsigned int>(new_data.key[i]); } cout<<endl; }
  
  
  for(i = 0; i < count; i++){
    splitDimsMatched = howManySplitDimsCovered( new_data, entriesVec.at(i).DMBR,  entriesVec[i].whichDimsSplit, allDim);
    if(allDim && splitDimsMatched>bestAllDimCount){
      bestAllDimChild = i;
      bestAllDimCount = splitDimsMatched;
    }
    else if(!allDim && bestAllDimChild<0 && splitDimsMatched>bestNotAllDimCount){
      notAllDimMaxChild = i;
      bestNotAllDimCount = splitDimsMatched;
    }
  }  
  
  if(bestAllDimChild>=0){
    child_index = bestAllDimChild;
  }
  else if(notAllDimMaxChild>=0){
    child_index = notAllDimMaxChild;
  }
  child_block_no = entriesVec[ child_index ].child;
  if(child_index<0) {cout<<"No child found"<<endl; exit(1);}
...
void Dir_node::find_best_child_sprt(const Leaf_entry &new_data, int* alphabet_sizes, unsigned int& child_block_no, int& child_index, int index, int cur_height){
  ...
  int i, j,k, maxSplitDimsMatched=-1, splitDimsMatched, best_child_index=-1;
  unsigned int best_block_no;
  bool logOutput = false, matchedCommonDims;
  SplitDimBitBool commonSplitDims,tempCommonDims;
  vector<int> bestChildern;
  int commonDims[DIM], splitDimSize=0;
  
  //if(logOutput){ for(i = 0; i < DIM; i++){ query_result_file<<setw(4)<<static_cast<unsigned int>(new_data.key[i]); } query_result_file<<endl; }
  if(logOutput){ for(i = 0; i < DIM; i++){ cout<<setw(4)<<static_cast<unsigned int>(new_data.key[i]); } cout<<endl; }
  
  commonSplitDims = (~(SplitDimBitBool)0);
  for(i = 0; i < count; i++){
    commonSplitDims &= entriesVec[i].whichDimsSplit;
  }
  tempCommonDims = commonSplitDims;
  
  if(logOutput) cout<<"commonSplitDims:"<<commonSplitDims<<endl;
  
  list_common_split_dims(tempCommonDims, commonDims, splitDimSize);
  
  if(splitDimSize>0){
    for(i = 0; i < count; i++){
      //if(logOutput){ for(j = 0; j < DIM; j++){ query_result_file<<entriesVec[i].DMBR[j]<<"||"; } query_result_file<<endl; }
      if(logOutput){ for(j = 0; j < DIM; j++){ cout<<static_cast<unsigned int>(entriesVec[i].DMBR[j])<<"||"; } cout<<endl; }
      
      if(logOutput) cout<<entriesVec[i].whichDimsSplit<<endl;
      
      //check if the DMBR cover all the commonSplitDims
      matchedCommonDims = is_covered_secleced_dim(new_data, entriesVec[i].DMBR, commonDims, splitDimSize);
      
      if(matchedCommonDims){
         splitDimsMatched = howManySplitDimsCovered( new_data, entriesVec.at(i).DMBR,  entriesVec[i].whichDimsSplit);
         //if(logOutput) query_result_file<<splitDimsMatched<<endl;
         if(logOutput) cout<<splitDimsMatched<<endl;
         
         if(splitDimsMatched > maxSplitDimsMatched){
           maxSplitDimsMatched = splitDimsMatched;
           best_child_index = i;
         }
      }    
    }  
    ...
    if(logOutput) cout<<"maxSplitDims:"<<maxSplitDimsMatched<<"\t child_index:"<<child_index<<"\t child_block_no:"<<child_block_no<<endl<<endl;
  }
  else{
    cout<<"No Split Dim available"<<endl;
    
    cout<<"fIndex:"<<index<<"\tCur_height"<<cur_height<<endl;
    for(i = 0; i < count; i++){
      cout<<i<<"::"<<entriesVec[i].whichDimsSplit<<"\t";
    }
    cout<<endl;
...
Error_code Dir_node::insert_new_entry_sprt(unsigned int new_entry_block_no, unsigned char* new_entry_DMBR, SplitDimBitBool newWhichDimsSplit, double dir_min_util, 
int *alphabet_sizes, unsigned char* cur_DMBR, Dir_node*& new_dir_node, unsigned char* new_DMBR, int& best_dir_dim){
  
  int i;
  
  Dir_entry tempEntry;
  tempEntry.child = new_entry_block_no;
  tempEntry.whichDimsSplit = newWhichDimsSplit;
  
  for(i = 0; i < DMBR_SIZE; i++)tempEntry.DMBR[i] = new_entry_DMBR[i];
  bool noOverFlow,logOutput=false;
  ...
  if(noOverFlow)
  {
    ...
    //return success;
    //cout<<"Before Addition:"<<count<<endl;
  ...
  else{ // overflows, split
    //Dir_entry tmp_entries[DIR_NODE_SIZE+1];
    //cout<<"dir node split"<<endl;
    ...
    if(logOutput){
      for(i=0; i<(count+1); i++){
        cout<<i<<"::"<<tmp_entries[i].whichDimsSplit<<"\t";
      }
      cout<<endl;
    }
    ...
    switch (nodeSplitType){
      ...
      case SPACE_PARTITION:
        //best_dir_dim = split_algorithm_sprt(tmp_entries, dir_min_util, alphabet_sizes, entry_group1, count1, entry_group2, count2, new_DMBR, cur_DMBR); // group1 corresponds new_DMBR. group2 corresponds cur_DMBR
        best_dir_dim = split_TO_DEATH_RANDOM_UTIL_v4(tmp_entries, dir_min_util, alphabet_sizes, entry_group1, count1, entry_group2, count2, new_DMBR, cur_DMBR);
        break;
        //case TO_DEATH:
    ...
    //cout<<"best_dir_dim:"<<best_dir_dim<<endl;
...
//   start = clock();
   ...
   unsigned int child_array[DIR_NODE_SIZE]; 
   unsigned char DMBR_array[DIR_NODE_SIZE][DMBR_SIZE];
   SplitDimBitBool whichDimsSplit_array[DIR_NODE_SIZE]; 
   
   
   ND_file.read((char*)child_array, sizeof(child_array));
   ND_file.read((char*)DMBR_array, sizeof(DMBR_array));
   ND_file.read((char*)whichDimsSplit_array, sizeof(whichDimsSplit_array));
...
//   cout << (double)(finish - start) / CLOCKS_PER_SEC << " ";
   ...
   for(int i = 0; i < count; i++){
    ...
    entriesVec.at(i).whichDimsSplit = whichDimsSplit_array[i];
...
void Dir_node::write_node_static(fstream& ND_file, unsigned int block_number){
   ...
   for(int i = 0; i < count; i++){
      ...
      whichDimsSplit_array[i] = entriesVec[i].whichDimsSplit;
   }

   ND_file.write((const char*)child_array, sizeof(child_array));
   ND_file.write((const char*)DMBR_array, sizeof(DMBR_array));
   ND_file.write((const char*)whichDimsSplit_array, sizeof(whichDimsSplit_array));
   
}

bool Dir_node::is_covered_secleced_dim(const Leaf_entry &query_data, unsigned char* DMBR, int* commonDims, int splitDimSize){
  
  int i,curDim;
  int byte_no, bit_no;
  
  for(i = 0; i < splitDimSize; i++){
    curDim = commonDims[i];
    byte_no = this->DMBR_byte_lut[curDim][query_data.key[curDim]];
    bit_no = this->DMBR_bit_lut[curDim][query_data.key[curDim]];
    if(!(DMBR[byte_no] & MASKS[bit_no]))return false; // one dim not covered by current entry, try next
  }
  return true;
...
//Knapsack problem
int Dir_node::split_TO_DEATH_RANDOM_UTIL_v4(Dir_entry* split_entries, double dir_min_util, int* alphabet_sizes, int* group1, int &count1, int* group2, int &count2, unsigned char* DMBR1, unsigned char* DMBR2)
{
   int i, j, k, best_dim;
...
void Dir_node::set_node_entry_whichDimSplit(int index, SplitDimBitBool newWhichDimsSplit){
  assert(index >= 0 && index < count);
  entriesVec.at(index).whichDimsSplit = newWhichDimsSplit;
}

SplitDimBitBool Dir_node::get_node_entry_whichDimSplit(int index){
  assert(index >= 0 && index < count);
  return entriesVec.at(index).whichDimsSplit;
}
...
void Dir_node::NDT_sort_subset(int count, bool asc, vector<unsigned int>& splitDims, vector<unsigned char>& splitDimsCount){
...
void Dir_node::recursiveUtilCal(int& alphabetSize, int minUtilReq, vector<unsigned char>& varAlphabetFreq, int index, bool* minSplitChars,
...
int Dir_node::greedyMinBalanceSplit(int& alphabetSize, vector<unsigned char>& varAlphabetFreq, bool* minSplitChars, double dir_min_util,
int curMaxAlphabetSize){
  
  int i,j,k,l, minUtilReq = ceil(dir_min_util*DIR_NODE_SIZE), curMinUtil = DIR_NODE_SIZE, newMinUtil,startPoint;
...
int Dir_node::split_algorithm_sprt(Dir_entry* split_entries, double dir_min_util, int* alphabet_sizes, int* group1, int &count1, int* group2, int &count2, unsigned char* DMBR1, unsigned char* DMBR2){
  
  SplitDimBitBool commonSplitDims,tempCommonDims;
  int i,j, sort_dim, splitDimSize=0, num_of_entries = count + 1,curDimMinUtil, globalMinUtil = DIR_NODE_SIZE+1, best_dim;
  int commonDims[DIM];
  bool logOutput = true;
  ...
  for(i = 0; i < num_of_entries; i++){
    //if(logOutput) cout<<split_entries[i].whichDimsSplit<<"\t"<<commonSplitDims<<endl;
    commonSplitDims &= split_entries[i].whichDimsSplit;
  }
  tempCommonDims = commonSplitDims; 
  list_common_split_dims(tempCommonDims, commonDims, splitDimSize);
  if(logOutput){cout<<"DirsplitDimSize:"<<splitDimSize<<endl; for(i=0; i<splitDimSize; i++) cout<<commonDims[i]<<"\t"; cout<<endl;}
  
  //for each common split dim, try to find space partition and finally best space partition
  ...
  int tmp_sum, size_of_alphabet2 = 0;
  int num_of_sort_entries = num_of_entries;
  
  
  vector<unsigned int> varLengthAlphabetIndex;
  vector<unsigned char> varAlphabetFreq;
  bool minSplitChars[ALPHA] = {false};
  vector<unsigned int>globalMinSplitChars;  //min util char list of first group
  vector<int> group1_entries;
  vector<int> group2_entries;
  ...
  for(i=0; i<splitDimSize; i++){
    ...
    sort_dim = commonDims[i];
    //check span here
    int dim_span = 0;
    for(j = this->DMBR_start_byte_lut[sort_dim]; j <= this->DMBR_end_byte_lut[sort_dim]; j++)
      dim_span += bit_1_count_lut[tmp_DMBR[j]];
    if(dim_span==1) {
      if(logOutput) cout<<sort_dim<<" span 1. so going ahead to next dim"<<endl;
      continue;
    }
    ...
    for(i = 0; i < num_of_sort_entries; i++){
      ...
      for(j = 0; j < num_of_set_entry; j++)
        if(set_equal(split_entries[i].DMBR+this->DMBR_start_byte_lut[sort_dim], set_entry_array[j].set, cur_dim_byte_size)){
          found = true;
          break;
        }
        if(found){
          ...
          for(j = 0; j < cur_dim_byte_size; j++){
            set_entry_array[num_of_set_entry].set[j] = split_entries[i].DMBR[this->DMBR_start_byte_lut[sort_dim]+j];
            tmp_sum += bit_1_count_lut[split_entries[i].DMBR[this->DMBR_start_byte_lut[sort_dim]+j]];
          }
          ...
          for(j = 0; j < cur_dim_byte_size; j++)tmp_set[j] |= split_entries[i].DMBR[this->DMBR_start_byte_lut[sort_dim]+j];
        }
    }
    ...
    if(logOutput){
      cout<<"size_of_alphabet2:"<<size_of_alphabet2<<endl;
      for(i=0; i<num_of_set_entry; i++){
        cout<< static_cast<unsigned int>(alphabet2[i])<<endl;
      }
      cout<<"num_of_set_entry:"<<num_of_set_entry<<endl;
      for(i=0; i<num_of_set_entry; i++){
        cout<<"Freq:"<<set_entry_array[i].freq<<"\tSize:"<<set_entry_array[i].element_size<<endl;
      }
    }
    //ends - group entries
    ...
    if(num_of_set_entry>1){
      ...
      varLengthAlphabetIndex.clear();
      varAlphabetFreq.clear();
      
      for(j=0; j<num_of_set_entry; j++){
        varLengthAlphabetIndex.push_back(j);
        varAlphabetFreq.push_back(set_entry_array[j].freq);
      }
      
      NDT_sort_subset(num_of_set_entry, false, varLengthAlphabetIndex, varAlphabetFreq);
      
      if(logOutput){ cout<<"After FreqSorting. Dim:"<<sort_dim<<endl; 
        for(j=0; j<num_of_set_entry; j++){ cout<<static_cast<unsigned int>(varLengthAlphabetIndex[j])<<"::"<<static_cast<unsigned int>(varAlphabetFreq[ j ])<<"\t"; } cout<<endl;
      }  
      
      curDimMinUtil = greedyMinBalanceSplit(num_of_set_entry, varAlphabetFreq, minSplitChars, dir_min_util, ALPHA);
      ...
      if(curDimMinUtil<globalMinUtil){
        group1_entries.clear();
        group2_entries.clear();
        best_dim = sort_dim;
        globalMinUtil = curDimMinUtil;
        globalMinSplitChars.clear(); //clear previous min util char list of first group
        for(j=0; j<num_of_set_entry; j++){ 
          tmp_index = varLengthAlphabetIndex[j];
          if(minSplitChars[j]){ 
            //cout<<"A"<<tmp_index<<"::"<<endl;
            //globalMinSplitChars.push_back(varLengthAlphabetIndex[j]);
            for(int nn=0; nn<set_entry_array[ tmp_index ].freq; nn++){
              group1_entries.push_back(set_entry_array[tmp_index].entry_set[nn]);
            }
          }
          else{
            //cout<<"B"<<tmp_index<<endl;
            for(int nn=0; nn<set_entry_array[ tmp_index ].freq; nn++){
              group2_entries.push_back(set_entry_array[tmp_index].entry_set[nn]);
            }
          }
  ...
  count1 = group1_entries.size();
  count2 = group2_entries.size();
  if(best_dim>=0){ //best dim found
    
    for(i=0; i<count1; i++) group1[i] = group1_entries[i];
    for(i=0; i<count2; i++) group2[i] = group2_entries[i];
    
    //for(i=0; i<count1; i++) cout<<group1_entries[i]<<"\t"; cout<<endl;
    //for(i=0; i<count2; i++) cout<<group2_entries[i]<<"\t"; cout<<endl;
    cout<<"globalMinUtil:"<<globalMinUtil<<"\tcount1:"<<count1<<endl;
  ...
  cal_DMBR(split_entries, group1, count1, DMBR1); // create DMBR for grp1
  cal_DMBR(split_entries, group2, count2, DMBR2); // create DMBR for grp2
  if(logOutput){
    cout<<endl;
    //printDMBR(DMBR1);
    //printDMBR(DMBR2);
