...
public:
   ...
   Error_code insert_use_link_sprt(Leaf_entry &new_data, int &number_of_io, int index);
     
   void print_information();

   unsigned int get_tree_size();

	void box_query( const Dir_entry box_query_data,	Leaf_entry* query_results, int &number_of_query_results,int & number_of_io, int &dir_io);
...
private:
   ...
   unsigned int choose_subtree_sprt(Leaf_entry &new_data, int &number_of_io, stack<unsigned int>& node_stack, stack<int>& index_stack, int index);
...
void ND_tree::box_query(const Dir_entry box_query_data, Leaf_entry* query_results, int &number_of_query_results, int &	number_of_io, int &dir_io){
  
	int i;   
	number_of_query_results = 0;
	number_of_io = 0, dir_io = 0;
    ...
    //cout<<"test"<<endl;
      ...
      dir_io++;
			...
			d_node->find_entries_by_box_query( box_query_data, block_numbers_in_box, tmp_height);
			...
			if(tmp_height == 2){ // One level higher than leaf
			  ...
			  //cout<<"test==2"<<endl;
				...
				for(i = 0; i < block_numbers_in_box.size(); i++){ // go through all the relevant entries
...
Error_code ND_tree::insert_use_link_sprt(Leaf_entry &new_data, int &number_of_io, int index){
  /////////////////////////
  //	/*debug*/
  //	if(height>1)
  //	{
  ...
  SplitDimBitBool curDimSplit=0, newDimSplit = 0; 
  Error_code result;
  stack<unsigned int> ins_path_nodes; // sequence of dir node block nos from root
  stack<int> ins_path_child_indices; // sequence of child indices from root
  unsigned int insert_block_no = choose_subtree_sprt(new_data, number_of_io, ins_path_nodes, ins_path_child_indices, index);
  ...
  int best_dim, newSplitDim,dir_best_dim;
  SplitDimBitBool newWhichDimsSplit = 0, level2WhichDimsSplit = 0, DirNode1WhichDimsSplit = 0, DirNode2WhichDimsSplit = 0; //to track which dim are split in the w-mers for dirEntry
  
  result = l_node->insert_new_data_sprt(new_data, leaf_min_util, alphabet_sizes, cur_DMBR, new_l_node, new_DMBR, best_dim); // new_l_node and new_DMBR used only if split
  l_node->write_node(ND_file, insert_block_no);
  number_of_io++;
  ...
  if(result == success){ // no overflow.  Adjust DMBR only
    ...
    //cout<<"best_dim:"<<best_dim<<endl;//exit(1);
    DirNode1WhichDimsSplit |= (SplitDimBitBool) 1 << best_dim; //set only current dimSplit, may be or-ed with existing dims later
    DirNode2WhichDimsSplit = DirNode1WhichDimsSplit;
    ...
    while(overflow_now && cur_level < height){ // not root yet
      ...
      if(cur_level==1){
        DirNode1WhichDimsSplit |= d_node->get_node_entry_whichDimSplit(child_index);
        DirNode2WhichDimsSplit = DirNode1WhichDimsSplit;
      }
      else{
        DirNode1WhichDimsSplit |= d_node->get_node_entry_whichDimSplit(child_index);
        DirNode2WhichDimsSplit = DirNode1WhichDimsSplit;
        //cout<<"Height:"<<cur_level<<"\t"<<DirNode1WhichDimsSplit<<"::"<<DirNode2WhichDimsSplit<<endl;
      }
      
      number_of_io++;
      d_node->set_DMBR(child_index, cur_DMBR, need_to_change);
      //update current leaf node dimsplit
      d_node->set_node_entry_whichDimSplit(child_index, DirNode1WhichDimsSplit);
      ...
      //result = d_node->insert_new_entry(new_block_no, new_DMBR, dir_min_util, alphabet_sizes, cur_DMBR, new_d_node, new_DMBR);
      dir_best_dim = 0;
      result = d_node->insert_new_entry_sprt(new_block_no, new_DMBR, DirNode2WhichDimsSplit, dir_min_util, alphabet_sizes, cur_DMBR, new_d_node, new_DMBR, dir_best_dim);
      ...
      if(result == success){ // no overflow
        ...
        //cout<<"ND_tree dir_best_dim:"<<dir_best_dim<<endl;
        DirNode1WhichDimsSplit = (SplitDimBitBool) 1 << dir_best_dim; //set only current dimSplit, may be or-ed with existing dims later
        DirNode2WhichDimsSplit = DirNode1WhichDimsSplit;
        //cout<<DirNode1WhichDimsSplit<<"\t"<<DirNode2WhichDimsSplit<<"\tIndex:"<<index<<endl;
    ...
    if(overflow_now){ // cur_level == height
      ...
      if(height==1){
        new_root_node->set_node_entry_whichDimSplit(0,DirNode1WhichDimsSplit);
        new_root_node->set_node_entry_whichDimSplit(1,DirNode2WhichDimsSplit);
      }
      else{
        //cout<<"NewRoot:"<<DirNode1WhichDimsSplit<<"\t"<<DirNode2WhichDimsSplit<<endl;
        new_root_node->set_node_entry_whichDimSplit(0, DirNode1WhichDimsSplit);
        new_root_node->set_node_entry_whichDimSplit(1, DirNode2WhichDimsSplit);
      }
...
void ND_tree::print_information(){
   ...
   if(nodeSplitType  == TO_DEATH_RANDOM_UTIL || nodeSplitType  == SPACE_PARTITION)
   {
...
unsigned int ND_tree::choose_subtree_sprt(Leaf_entry &new_data, int &number_of_io, stack<unsigned int>& node_stack, stack<int>& index_stack, int index){
  ...
  while(cur_height > 1){
    ...
    if(PartitionType==1){
      d_node->find_best_child_sprt(new_data, alphabet_sizes, cur_block_no, cur_child_index, index,cur_height);
    }
